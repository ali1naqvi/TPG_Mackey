...
def RunBestAgent(args):
    ...
    simulated_data.to_csv('Simulation_1.txt', index=False)

if __name__ == '__main__':
    tStart = time.time()
    trainer_checkpoint_path = Path("trainer_savepoint_1.pkl")
    gen_checkpoint_path = Path("gen_savepoint_1.txt")
    ...
    with open('results_1.txt', 'a' if gen_start > 0 else 'w') as file:
        ...
        for gen in range(gen_start, GENERATIONS): 
            ...
            champ.saveToFile("best_agent_1")

            trainer.evolve(tasks=['main'])
            
            validation_champion_path = Path("validation_champion_1")
            if gen % 10 == 0 and gen != 0:  # Validation phase every 10 generations
                ...
                while looper:
                    ...
                    if current_best_validation.team.outcomes['validation'] >= prevbestscore:
                        ...
                        validationChamp.saveToFile("validation_champion_1")
                    else: 
                        ...
                        with open("final_validation_scores_1.txt", 'w') as f:
            ...
            trainer.saveToFile("trainer_savepoint_1.pkl")
            with open("gen_savepoint_1.txt", 'w') as gen_file:
        ...
        champ = pickle.load(open("best_agent_1", 'rb'))
